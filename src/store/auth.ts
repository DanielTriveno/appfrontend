import {
  ...
  const initialState: AuthState = {
    isAuthenticated:
      (getLocalStorage('isAuthenticated') || getSessionStorage('isAuthenticated')) === 'true' &&
  ...
  const authSlice = createSlice({
    ...
    initialState,
    reducers: {
      login: (state, action: PayloadAction<{ user: User; access: string; refresh?: string; remember: boolean }>) => {
